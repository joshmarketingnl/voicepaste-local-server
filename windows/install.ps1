...
#
# Usage (PowerShell):
#   .\install.ps1 [-Model small|turbo] [-Port 8765] [-AutoStart] [-Uninstall]
#
param(
...
$ServerExe = Join-Path $InstallDir 'bin\whisper-server.exe'
if (-not (Test-Path $ServerExe)) {
  ...
  Expand-Archive $zip -DestinationPath (Join-Path $InstallDir 'bin') -Force
...
$cmdScript = @"
@echo off
rem VoicePaste lokale transcriptie-server (whisper.cpp)
cd /d "%~dp0"
echo Server draait op http://127.0.0.1:$Port/v1 - venster sluiten = server stopt.
"%~dp0bin\whisper-server.exe" -m "%~dp0models\$ModelFile" -l auto -t 8 --host 127.0.0.1 --port $Port --convert --inference-path /v1/audio/transcriptions --vad --vad-model "%~dp0models\$VadFile"
pause
"@
...
cmd = """" & scriptDir & "bin\whisper-server.exe""" & _
